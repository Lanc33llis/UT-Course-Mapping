majorNameToCode = {
	"Accounting": "ACC",
	"Advertising": "ADV",
	"Aerospace Engineering": "ASE",
	"African/African Diaspora": "AFR",
	"Air Force Science": "AFS",
	"American Sign Language": "ASL",
	"American Studies": "AMS",
	"Ancient History/Classical Civ": "AHC",
	"Anthropology": "ANT",
	"Applied Learning/Development": "ALD",
	"Arabic": "ARA",
	"Architectural Engineering": "ARE",
	"Architectural Interior Design": "ARI",
	"Architecture": "ARC",
	"Art Education": "AED",
	"Art History": "ARH",
	"Art, Studio": "ART",
	"Arts/Entertainment Technology": "AET",
	"Asian American Studies": "AAS",
	"Asian Studies": "ANS",
	"Astronomy": "AST",
	"Bassoon": "BSN",
	"Bengali": "BEN",
	"Biochemistry": "BCH",
	"Biology": "BIO",
	"Biomedical Engineering": "BME",
	"Bridging Disciplines": "BDP",
	"Business Administration": "B A",
	"Business Analytics": "BAX",
	"Business, Government, and Soci": "BGS",
	"Chemical Engineering": "CHE",
	"Chemistry": "CH",
	"Chinese": "CHI",
	"Civil Engineering": "C E",
	"Clarinet": "CLA",
	"Classical Civilization": "C C",
	"Cognitive Science": "CGS",
	"Communication": "COM",
	"Communication and Leadership": "CLD",
	"Communication Studies": "CMS",
	"Community/Regional Planning": "CRP",
	"Comparative Literature": "C L",
	"Computational Engineering": "COE",
	"Computational Sci, Engr/Math": "CSE",
	"Computer Science": "C S",
	"Conducting": "CON",
	"Core Texts and Ideas": "CTI",
	"Creative Writing": "CRW",
	"Curriculum and Instruction": "EDC",
	"Czech": "CZ",
	"Danish": "DAN",
	"Data Science": "DSC",
	"Decision Science": "D S",
	"Design": "DES",
	"Developmental Studies": "DEV",
	"Double Bass": "D B",
	"Drum Set": "DRS",
	"Dutch": "DCH",
	"Economics": "ECO",
	"Educational Leadership and Pol": "ELP",
	"Educational Psychology": "EDP",
	"Electrical & Computer Engr": "E E",
	"Electrical/Computer Engr": "ECE",
	"Energy and Earth Resources": "EER",
	"Engineering Management": "ENM",
	"Engineering Mechanics": "E M",
	"Engineering Studies": "E S",
	"English": "E",
	"English As A Second Language": "ESL",
	"Ensemble": "ENS",
	"Environmental Engineering": "EVE",
	"Environmental Science": "EVS",
	"Euphonium": "EUP",
	"European Studies": "EUS",
	"Finance": "FIN",
	"Fine Arts": "F A",
	"Flute": "FLU",
	"French": "FR",
	"French Civilization": "F C",
	"French Horn": "F H",
	"General Engineering": "G E",
	"Geography": "GRG",
	"Geological Sciences": "GEO",
	"German": "GER",
	"German, Scandinavian/Dutch": "GSD",
	"Government": "GOV",
	"Graduate School": "GRS",
	"Greek": "GK",
	"Guitar": "GUI",
	"Harp": "HAR",
	"Health and Society": "H S",
	"Health Care Transformation": "HCT",
	"Health Education": "HED",
	"Hebrew": "HEB",
	"Hindi": "HIN",
	"History": "HIS",
	"Human Dev and Family Sciences": "HDF",
	"Human Dimensions of Orgs": "HDO",
	"Human Ecology": "H E",
	"Humanities": "HMN",
	"Iberian/Latn Amer Lang/Cultrs": "ILA",
	"Indigenous Langs of Latn Amer": "LAL",
	"Informatics": "I",
	"Information Security/Privacy": "ISP",
	"Information Studies": "INF",
	"Integrated Design": "ITD",
	"International Business": "I B",
	"Intl Relations/Glbl Studies": "IRG",
	"Islamic Studies": "ISL",
	"Italian": "ITL",
	"Italian Civilization": "ITC",
	"Japanese": "JPN",
	"Jewish Studies": "J S",
	"Journalism": "J",
	"Kinesiology": "KIN",
	"Korean": "KOR",
	"Landscape Architecture": "LAR",
	"Lang Teaching/Coordination": "LTC",
	"Latin": "LAT",
	"Latin American Studies": "LAS",
	"Law": "LAW",
	"Legal Environment of Business": "LEB",
	"Liberal Arts": "L A",
	"Liberal Arts Honors": "LAH",
	"Linguistics": "LIN",
	"Malayalam": "MAL",
	"Management": "MAN",
	"Management Information Systems": "MIS",
	"Manufacturing Systems Engineer": "MFG",
	"Marine Science": "MNS",
	"Marketing": "MKT",
	"Materials Science and Engineer": "MSE",
	"Mathematics": "M",
	"Mechanical Engineering": "M E",
	"Medieval Studies": "MDV",
	"Mexican American Studies": "MAS",
	"Middle Eastern Langs/Cultures": "MEL",
	"Middle Eastern Studies": "MES",
	"Military Science": "M S",
	"Molecular Biology": "MOL",
	"Music": "MUS",
	"Natural Sciences": "NSC",
	"Naval Science": "N S",
	"Neuroscience": "NEU",
	"Norwegian": "NOR",
	"Nursing": "N",
	"Nutrition": "NTR",
	"Oboe": "OBO",
	"Opera": "OPR",
	"Operations Management": "O M",
	"Operations Research/Indstrl Engr": "ORI",
	"Organ": "ORG",
	"Percussion": "PER",
	"Persian": "PRS",
	"Petroleum/Geosystems Engr": "PGE",
	"Pharmacy Graduate Studies": "PGS",
	"Pharmacy PharmD": "PHM",
	"Philosophy": "PHL",
	"Physical Education": "PED",
	"Physical Science": "P S",
	"Physics": "PHY",
	"Piano": "PIA",
	"Polish": "POL",
	"Portuguese": "POR",
	"Portuguese Civilization": "PRC",
	"Psychology": "PSY",
	"Public Affairs": "P A",
	"Public Health": "PBH",
	"Public Relations": "P R",
	"Race, Indigeneity/Migration": "RIM",
	"Radio-Television-Film": "RTF",
	"Real Estate": "R E",
	"Religious Studies": "R S",
	"Rhetoric and Writing": "RHE",
	"Risk Management": "R M",
	"Rus, East Euro/Eurasian Studies": "REE",
	"Russian": "RUS",
	"Sanskrit": "SAN",
	"Saxophone": "SAX",
	"Sci/Technology Commercialization": "STC",
	"Science": "SCI",
	"Science, Technology, Engr": "STM",
	"Serbian/Croatian": "S C",
	"Slavic/Eurasian Languages": "SEL",
	"Social Science": "S S",
	"Social Work": "S W",
	"Sociology": "SOC",
	"Spanish": "SPN",
	"Spanish Civilization": "SPC",
	"Special Education": "SED",
	"Speech, Language, and Hearing": "SLH",
	"Statistics": "STA",
	"Statistics and Data Sciences": "SDS",
	"Sustainability Studies": "SUS",
	"Swedish": "SWE",
	"Tamil": "TAM",
	"Textiles and Apparel": "TXA",
	"Theatre and Dance": "T D",
	"Trombone": "TRO",
	"Trumpet": "TRU",
	"Tuba": "TBA",
	"Turkish": "TUR",
	"Tutorial Course": "T C",
	"Ukrainian": "UKR",
	"Undergraduate Studies": "UGS",
	"Urban Design": "UDN",
	"Urban Studies": "URB",
	"Urdu": "URD",
	"Uteach-Liberal Arts": "UTL",
	"Uteach-Natural Sciences": "UTS",
	"Viola": "VIA",
	"Violin": "VIO",
	"Violoncello": "V C",
	"Visual Art Studies": "VAS",
	"Voice": "VOI",
	"Women'S and Gender Studies": "WGS",
	"Writing": "WRT",
	"Yiddish": "YID",
	"Yoruba": "YOR",
}